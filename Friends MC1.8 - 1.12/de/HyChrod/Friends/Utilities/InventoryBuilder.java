...
public enum InventoryBuilder {
	...
	public static HashMap<String, Friendship> openFriendInventory(Player player, UUID toOpen, int page, boolean fresh) {
		...
		Bukkit.getScheduler().runTaskAsynchronously(Friends.getInstance(), new Runnable() {
			...
			public void run() {
				...
				LinkedList<Friendship> friends = new LinkedList<Friendship>(fresh ? hash.getFriendsNew() : hash.getFriends());
				...
				if(Configs.INV_FRIEND_NEXTPAGE_ENABLE.getBoolean() && (!Configs.INV_FRIEND_HIDEPAGES.getBoolean() || (Configs.INV_FRIEND_HIDEPAGES.getBoolean() && friends.size() > (freeSlots-1)))) {
				...
				for (Friendship fs : friendsOnPage) {
					...
					status = status == null || fs.getStatus() == null || fs.getStatus().length() < 1 ? Configs.INV_FRIENDS_NO_STATUS_REPLACEMENT.getText() : status;
	...
	private static ItemStack createHead(String identifier, UUID uuid, String name, String display, String lore, String[] toReplace, String[] replacements, boolean changeSkull) {
		ItemStack item = new ItemStack(Material.SKULL_ITEM, 1, changeSkull ? (short)(3) : (short)0);
		...
		if(changeSkull) meta.setOwner(name);
