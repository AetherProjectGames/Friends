...
public enum ItemStacks {
	...
	private static void generateCustomItems(FileConfiguration cfg) {
		...
		for(String inv : inventorys) {
			...
			for(String cFItems : cfg.getConfigurationSection("Friends." + inv + ".CustomItems").getKeys(false)) {
				...
				values[1] = cfg.getString("Friends." + inv + ".CustomItems." + cFItems + ".ItemID");
	...
	public static ItemStack getCutomItem(String inv, int index, OfflinePlayer p) {
		...
		int bytData = 0;
		if(id.length > 1)
			bytData = Integer.parseInt(id[1]);
		return getItem(itemid, bytData, p == null ? data[0] : data[0].replace("%NAME%", p.getName()), data[2].length() == 0 ? new ArrayList<String>() : Arrays.asList((p == null ? data[2] : data[2].replace("%NAME%", p.getName())).split("//")), p, base64.length() > 20, base64);
	}
